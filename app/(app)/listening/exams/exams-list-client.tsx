...
import type { ListeningExam } from "@/lib/supabase/types";
import { useTheme, getColors } from "@/lib/use-theme";

type ExamWithStats = ListeningExam & {
  ...
  return (
    <div style={{ background: c.background, color: c.onSurface, minHeight: "100vh" }}>
      <main style={{ padding: "24px 24px 128px", maxWidth: 448, margin: "0 auto" }}>
        <Link href="/listening" style={{ display: "inline-flex", alignItems: "center", gap: 6, textDecoration: "none", color: c.onSurfaceVariant, fontSize: 13, fontWeight: 700, marginBottom: 16 }}>
          <span className="mso" style={{ fontSize: 18 }}>chevron_left</span>
          Terug naar Luisteren
        </Link>

        <h1 style={{ fontSize: 28, fontWeight: 800, color: c.primary, letterSpacing: "-0.025em", margin: 0 }}>
          Oefenexamens
        </h1>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {exams.map((exam) => {
            ...
            return (
              <Link key={exam.id} href={`/listening/exams/${exam.id}`} style={{ textDecoration: "none" }}>
                <div style={{
                  background: c.surfaceLowest, padding: 20, borderRadius: 20,
                  display: "flex", flexDirection: "column", gap: 12,
                  boxShadow: "0 4px 16px rgba(26,28,27,0.06)",
                  border: completed && exam.passed ? `2px solid ${c.primary}` : `1px solid ${c.outlineVariant}`,
                  cursor: "pointer",
                }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: c.primary, marginBottom: 4 }}>
                    ...
                    {completed && exam.passed && (
                      <span className="mso mso-fill" style={{ color: c.primary, fontSize: 28 }}>verified</span>
                    )}
                  </div>

                  <div style={{ display: "flex", gap: 16, fontSize: 12, fontWeight: 700, color: c.onSurfaceVariant }}>
                  ...
                  {completed && (
                    <div style={{
                      ...
                      <span style={{ fontSize: 12, fontWeight: 700, color: c.onSurfaceVariant }}>
                        Beste score: <span style={{ color: exam.passed ? c.primary : c.error, fontWeight: 900 }}>{exam.bestScore}%</span>
                        {" · "}{exam.attempts} pogingen
                      </span>
                      <span style={{
                        padding: "4px 10px", borderRadius: 9999, fontSize: 10, fontWeight: 800,
                        textTransform: "uppercase", letterSpacing: "0.05em",
                        background: exam.passed ? `${c.primary}1a` : `${c.error}1a`,
                        color: exam.passed ? c.primary : c.error,
                      }}>
