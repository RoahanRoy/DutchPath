...
export function ExamRunner({ exam, sections, userId }: Props) {
  ...
  const submit = async () => {
    ...
    // XP + streak + daily activity + writing-track counters — independent
    // writes, so run them in one parallel wave. Amsterdam-local date matches
    ...
    await Promise.all([
      ...
      sb.rpc("increment_track_stats", {
        p_user_id: userId, p_track: "writing", p_xp: xpAwarded, p_completed: 1,
      }),
    ]);
